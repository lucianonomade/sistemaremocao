    ...
    <div className="space-y-8 animate-in pb-20">
      ...
      {isDownloadModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-[#161B22] border border-[#30363D] rounded-[2.5rem] w-full max-w-xl p-8 shadow-2xl animate-in zoom-in-95 relative flex flex-col max-h-[90vh]">
            <button onClick={() => setIsDownloadModalOpen(false)} className="absolute top-6 right-6 text-[#8B949E] hover:text-white"><AlertCircle className="rotate-45" size={24} /></button>
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-[#8B949E]">
                {modalMode === 'export'
                  ? 'Filtre por lote, data e órgãos para gerar sua planilha.'
                  : 'Selecione os dados para atualizar o status diretamente no sistema.'}
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-6 pr-2 scrollbar-thin">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-[#8B949E] uppercase ml-1">Órgãos Incluídos</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries(downloadOrgans).map(([key, value]) => (
                    <label key={key} className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all group ${value ? 'bg-[#B8860B]/10 border-[#B8860B]/30' : 'bg-[#0D1117] border-[#30363D]'}`}>
                      <input
                        type="checkbox"
                        checked={value}
            ...
            <button
              ...
              disabled={isLoading}
              className={`mt-8 w-full ${modalMode === 'export' ? 'bg-[#B8860B]' : 'bg-green-600'} hover:opacity-90 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50`}
            >
              {modalMode === 'export' ? (
                <>
                ...
                <>
                  <ShieldCheck size={20} />
                  {isLoading ? 'Processando...' : 'Confirmar Baixa no Sistema'}
                </>
              )}
            </button>
          </div>
        ...
        {groupedItems.map((item: any) => {
          ...
          return (
            <div key={displayId} className={`bg-[#161B22] border rounded-[2.5rem] p-8 flex flex-col gap-6 shadow-2xl relative group transition-all ${isBatch ? 'border-[#B8860B]/40 bg-gradient-to-br from-[#161B22] to-[#1c180d]' : 'border-[#30363D] hover:border-[#B8860B]/30'}`}>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {isBatch && (
                    ...
                    >
                      <ListChecks size={14} />
                      {expandedBatches.includes(displayId) ? 'Ocultar CPFs' : 'Ver CPFs do Lote'}
                    </button>
                  )}
                </div>
              ...
              {isBatch && expandedBatches.includes(displayId) && (
                <div className="bg-[#0D1117] rounded-3xl border border-[#30363D] overflow-hidden animate-in slide-in-from-top-4 duration-300">
                  <div className="p-4 border-b border-[#30363D] bg-[#161B22]/50">
                    <h5 className="text-[10px] font-black text-[#8B949E] uppercase tracking-widest flex items-center gap-2">
                      <UserCheck size={14} className="text-[#B8860B]" />
                      Membros do Lote ({item.members.length})
                    </h5>
                  </div>
                  <div className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-[#30363D]">
                    {item.members.map((m: any) => (
                      <div key={m.id} className="p-4 border-b border-[#30363D]/50 flex justify-between items-center hover:bg-[#161B22]/30 transition-colors last:border-0">
                        <div>
                          <p className="text-xs font-bold text-white uppercase">{m.clientName || 'Cliente sem nome'}</p>
                          <p className="text-[10px] text-[#8B949E] font-medium">{m.clientDocument}</p>
                        </div>
                        <button
                          onClick={() => copyClientLink(m.id, m.clientDocument || '')}
                          className="p-2 rounded-xl bg-[#161B22] border border-[#30363D] text-[#8B949E] hover:text-[#B8860B] hover:border-[#B8860B]/30 transition-all"
                          title="Copiar Link"
                        >
                          <Share2 size={14} />
                        </button>
